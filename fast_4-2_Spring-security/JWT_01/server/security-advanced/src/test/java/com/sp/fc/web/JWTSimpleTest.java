...
public class JWTSimpleTest {
    ...
    void test_1() {
        ...
        Jws<Claims> tokenInfo = Jwts.parser().setSigningKey("pungwon").parseClaimsJws(okta_token);
    ...
    void test_2() {
        ...
        byte[] SEC_KEY = DatatypeConverter.parseBase64Binary("pungwon");

        String oauth0_token = JWT.create().withClaim("name", "pungwon").withClaim("price", 3000)
    ...
    @Test
    void test_3() {
        Algorithm AL = Algorithm.HMAC256("pungwon");
        String token = JWT.create().withSubject("a1234")
                .withExpiresAt(new Date(System.currentTimeMillis() + 1000))
                .withNotBefore(new Date(System.currentTimeMillis() + 3000))
                .sign(AL);
        try {
